class HubriseApp::HrUser < HubriseApp::ApplicationRecord
  self.table_name = :hr_users

  has_many :hr_user_app_instances, ->{ fresh }
  ...
  class << self
    ...
    def refresh_or_create_via_api_client(api_client, app)
      hr_attrs  = fetch_hr_attrs(api_client)
      hr_user   = find_or_initialize_by(hr_id: hr_attrs[:hr_id])
      hr_user.refresh(hr_attrs)
      hr_user
    end
  end

  def assign_hr_app_instance(hr_app_instance)
    user_app_instance = HrUserAppInstance.find_or_initialize_by(hr_app_instance_id: hr_app_instance.hr_id, hr_user_id: self.hr_id)
    user_app_instance.refresh!
  end
end
